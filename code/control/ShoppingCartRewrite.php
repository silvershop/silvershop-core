	...
	*******************************************************/
	...
	protected function currentOrder(){
		if (!$this->order) {
			...
			$this->order = DataObject::get_by_id('Order',Session::get(self::$session_variable.".ID")); //find order by id saved to session (allows logging out and retaining cart contents)
			if(!$this->order){
