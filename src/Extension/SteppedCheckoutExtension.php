...
class SteppedCheckoutExtension extends Extension
{
    ...
    {
        $action = $this->getOwner()->getRequest()->param('Action');
        $steps = $this->getSteps();
        if (!ShoppingCart::curr() instanceof Order && !empty($action) && isset($steps[$action])) {
            Controller::curr()->redirect($this->getOwner()->Link());
            return;
        }
    }
    ...
    {
        if ($this->getOwner()->getAction() === $name) {
            return true;
        }

        if (!$this->getOwner()->getAction() || $this->getOwner()->getAction() === 'index') {
    ...
    {
        return $this->compareActions($name, $this->getOwner()->getAction()) < 0;
    }
     ...
     */
    public function index(): HTTPResponse|array|null
    {
        if (CheckoutPage::config()->first_step) {
            return $this->getOwner()->{CheckoutPage::config()->first_step}();
        }
     ...
     */
    private function compareActions($action1, $action2): int
    {
     ...
     */
    private function actionPos($incoming): ?int
    {
        $count = 0;
