<% if Cart %>
	...
	<div id="ShoppingCart">
		...
		<table id="InformationTable" class="editable" cellspacing="0" cellpadding="0" summary="<% _t("Cart.TABLESUMMARY","The contents of your cart are displayed in this form and summary of all fees associated with an order and a rundown of payments options.") %>">
			<tbody>
				<% if Items %>
					<% control Items %>
						<% if ShowInTable %>
							<tr id="$TableID" class="$Classes hideOnZeroItems">
								<td <% if Link %><% else %> id="$TableTitleID"<% end_if %> class="product title" scope="row">
									<% if Link %>
										<a id="$TableTitleID" href="$Link" title="<% sprintf(_t("Cart.READMORE","Click here to read more on &quot;%s&quot;"),$TableTitle) %>">$TableTitle</a>
									<% else %>
										$TableTitle
									<% end_if %>
								</td>
					...
					<tr class="gap total summary hideOnZeroItems">
						<td colspan="2" scope="row"><% _t("Cart.TOTAL","Total") %></td>
						<td class="right total" id="$TableTotalID">$Total.Nice $Currency</td>
					</tr>
					<tr class="showOnZeroItems"<% if Items %> style="display: none"<% end_if %>>
						<td colspan="3" scope="row" class="center"><% _t("Cart.NOITEMS","There are <strong>no</strong> items in your cart.") %></td>
					</tr>
			</tbody>
		</table>
