...
//===================---------------- START ecommerce MODULE ----------------===================
		...
		products\Product::set_global_allow_purcahse($allow = false){
		products\Product::set_thumbnail_size($width = , $height = ){
		products\Product::set_content_image_width($width = ){
		products\Product::set_large_image_width($width = ){
		products\ProductGroup::set_include_child_groups($include = true){
		products\ProductGroup::set_page_length($length){
		products\ProductGroup::set_must_have_price($must = true){
		products\ProductGroup::set_sort_options(array $options){

		search\filters\OrderFilters::set_how_many_days_around($v){self::$how_many_days_around = $v;}

		tasks\HourlyEcommerceGroupUpdate::set_group_name($v) {self::$group_name = $v;}
